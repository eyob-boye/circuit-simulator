...
def read_circuit_parameters(nw_input, conn_matrix, branch_params, component_objects,  components_found):
    ...
    for c1 in range(len(nw_input)):
        print "%s ---> %s" %(nw_input[c1]+".csv", parameters_file[c1])
    
    while cont_ans.lower()!="y":
        cont_ans=raw_input("Enter parameters in files above. When done, \
...
def update_control_django(sim_para_model, component_objects, components_found, \
    ...
    # It will contain the individual control
    # codes as functions.
    output_control_path = os.path.join(os.sep, \
                                sim_para_model.sim_working_directory, \
                                "__control.py")
    complete_control = open(output_control_path,"w")
...
#        control_files[c1] = control_files[c1]+".py"
    ...
    # A list of all control files with full path
    control_files = []
    ...
    for c1 in range(len(control_file_list)):
        control_file_path = os.path.join(os.sep, \
                                sim_para_model.sim_working_directory, \
                                control_file_list[c1].control_file_name)
        control_files.append(control_file_path)

        control_functions.append(\
                control_file_list[c1].control_file_name.split(".py")[0]+"_func")
        
        control_file_item = control_file_list[c1]
        # Adding an empty dictionary for a control file.
        control_file_inputs.append({})
        control_file_outputs.append({})
        control_file_staticvars.append({})
        ...
        for contol_input_item in control_input_list:
            meter_type = contol_input_item.input_source
            ...
            # dictionary key for control_file_inputs.
            for c3 in range(len(components_found[meter_type.split("_")[0].lower()])):
                if components_found[meter_type.split("_")[0].lower()][c3][1]==\
                                meter_type.split("_")[1]:
                    meter_type_ref = meter_type.split("_")[0].lower()
                    control_file_inputs[c1][components_found[meter_type_ref][c3][0]]=\
                                [components_found[meter_type_ref][c3][1]]

                    var_name = contol_input_item.input_variable_name
        ...
        for contol_output_item in control_output_list:
            element_type = contol_output_item.output_target

            # Look for the controlled element in components_found
            # and get the cell position from the device tag.
            # The cell position will be the unique dictionary key
            for c3 in range(len(components_found[element_type.split("_")[0].lower()])):
                if components_found[element_type.split("_")[0].lower()][c3][1]==\
                            element_type.split("_")[1]:
                    element_type_ref = element_type.split("_")[0].lower()
                    comp_pos_3D = components_found[element_type_ref][c3][0]
                    # Since a controlled element can have more than one control input
                    # Check if it has been found before.
                    if not components_found[element_type_ref][c3][0] in \
                                control_file_outputs[c1].keys():
                        control_file_outputs[c1][components_found[element_type_ref][c3][0]] = \
                                [components_found[element_type_ref][c3][1]]

                    ckt_file_list = sim_para_model.circuitschematics_set.all()
                    for ckt_file_item in ckt_file_list:
                        comp_model = component_objects[comp_pos_3D].\
                                list_existing_components(ckt_file_item)
                        if comp_model:
                            break

                    if comp_model:
                        control_tag_name = comp_model.comp_control_tag
                    else:
                        control_tag_name = "Control"

                    control_var_name = contol_output_item.output_variable_name
                    control_init_value = contol_output_item.output_initial_value

                    control_file_outputs[c1][components_found[element_type_ref][c3][0]].\
                                append([control_tag_name, control_var_name, \
                                float(control_init_value)])
        ...
        for control_staticvar_item in control_staticvar_list:
            staticvar_type = control_staticvar_item.static_variable_name
            staticvar_val = control_staticvar_item.static_initial_value
            control_file_staticvars[c1][staticvar_type]=float(staticvar_val)
        ...
        for control_timeevent_item in control_timeevent_list:
            timeevent_type = control_timeevent_item.time_event_name
            timeevent_val = control_timeevent_item.initial_time_value
            control_file_timeevents[c1][timeevent_type] = float(timeevent_val)
    ...
    for control_varstore_item in sim_para_model.controlvariablestorage_set.all():
        # If it is a variable storage, the dictionary key
        # will be the variable name.
        varstore_name = control_varstore_item.variable_storage_name
        varstore_val = control_varstore_item.storage_initial_value
        varstore_plot = control_varstore_item.storage_status
        control_file_variablestorage[varstore_name]=[varstore_val, varstore_plot]
...
#    return
    return [control_files, control_functions, control_file_inputs, \
