    ...
    while ckt_param_exists=="no":
        try:
            ...
            while cont_ans.lower()!="y":
                ...
                cont_ans = raw_input("When done, close the file and press y and enter to continue -> ")
    ...
    if ckt_param_exists=="yes":
        ...
        for c1 in range(len(control_files)):
            ...
            if control_files[c1]!="" and control_files[c1]!=" ":
                ...
                if not len(extract_filename)==2:
                    print "Error. Control file %s is not a .py file." %control_files[c1]
                    sys.exit(1)
        ...
        for file_item in file_name_list:
            try:
                ...
                cont_ans = "p"
                while cont_ans.lower()!="y" or cont_ans.lower()!="n":
                    print "Output data file %s exists from a previous \
simulation." %file_item
                    cont_ans = raw_input("Would you like to abort now and make a backup or change the \
name of the output data file in circuit_inputs.csv? (Y/N) ")
    ...
    for c1 in range(len(nw_layout)):
        try:
            ...
            print "File %s cannot be read. Make sure \
file name is correct, and in the same \
folder/directory as circuit_solver.py" %nw_layout[c1]
            print
            sys.exit(1)
...
#    if controlled_elements:
    ...
    for count1 in range(len(meter_list)):
        print count1 + 2, 
        print " => ", 
        print component_objects[meter_list[count1]].type + "_" + component_objects[meter_list[count1]].tag, 
        print "at ", 
        print component_objects[meter_list[count1]].pos
    print
    if plotted_variable_list:
        ...
        for count2 in range(len(plotted_variable_list)):
            ...
            print " => ", 
            print plotted_variable_list[count2]
...
#                if branch_events[c1]=="yes" or branch_events[c1]=="hard" or branch_params[c1][-1][0][1]:
    ...
    print "Data stored in %s. Check output log file output_log.txt for description of output" %op_dat_file
